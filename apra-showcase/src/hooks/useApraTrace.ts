...
import type { ApraClientTraceRow, ApraRoundSummaryRow, ReplayFrame, AttackMethod, DatasetType } from "../types";
import { getTracePath, getRoundSummaryPath } from "../lib/data-paths";
import { parseCsv } from "../lib/csv";

export function useApraTrace(attack: AttackMethod, dataset: DatasetType) {
  ...
  useEffect(() => {
    Promise.all([
      fetch(getTracePath(attack, dataset)).then((r) => r.text()),
      fetch(getRoundSummaryPath(attack, dataset)).then((r) => r.text()),
    ])
