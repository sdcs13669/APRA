...
本作品提出一种基于自适应渐进鲁棒聚合的联邦学习后门对抗防御算法，简称为 APRA。APRA 的核心思想是将恶意客户端检测问题转化为多维度证据融合与渐进式降权问题：首先通过多维特征提取，将参数更新展平、L2 范数、NBD/NDIF 行为探针等多源信息转化为可比较的数值特征；随后依次通过自适应 MAD 预过滤、层次聚类与可信簇选择、信任加权与自适应裁剪三个阶段，逐步缩小可疑客户端的聚合贡献。
...
联邦学习允许多个参与方在不共享原始数据的前提下协同训练机器学习模型 [6]。其工作流程为：服务端将当前全局模型分发给被选中的客户端；各客户端使用本地数据训练模型，计算模型权重的变化量，即模型更新；客户端将更新上传至服务端；服务端聚合所有更新，得到新一轮全局模型。由于原始数据始终保留在本地，这一模式天然契合日益严格的数据隐私法规，已在智能手机输入法优化、医疗影像分析、金融风控等场景中得到部署。

然而，联邦学习的分布式架构也打开了新的攻击面。由于服务端无法直接审查客户端的本地数据和训练过程，恶意客户端可以提交精心构造的模型更新，在不显著影响全局模型主任务表现的前提下，向模型中植入后门行为 [1]。一个典型的后门攻击场景是：攻击者在本地训练时将像素模式、水印等特定触发器与目标类别关联，使得全局模型在遇到带触发器的输入时稳定输出攻击者指定的错误类别，而在干净样本上表现正常。这种隐蔽性使得后门攻击成为联邦学习面临的最严峻安全威胁之一。

后门攻击的危害跨越多个关键应用领域。在自动驾驶场景中，被植入后门的模型可能在识别到特定路标图案时将停车标志误分类为限速标志；在医疗诊断系统中，带有特定纹理的医学影像可能被系统性地误诊为目标疾病；在金融风控中，带有特定交易特征的欺诈行为可能被模型系统性放过。这些攻击不仅威胁系统安全，更可能造成人身伤害和重大经济损失。从技术角度看，联邦学习后门攻击具有三个令人担忧的特征：第一，攻击者只需控制少量客户端即可实现高成功率攻击；第二，攻击可以与正常训练同时进行，服务端难以从准确率指标上察觉异常；第三，持久性攻击在攻击停止后仍能使后门长期残留，增加了防御和清除的难度。

=== 联邦学习后门攻击方法

近年来，联邦学习后门攻击的手段日趋隐蔽，本作品覆盖了五种代表性攻击。

模型替换攻击（Model Replacement）由 Bagdasaryan 等人提出 [1]：攻击者首先在本地训练一个带后门的模型，然后将恶意更新的权重乘以一个大于 1 的系数进行放大，使其在服务端聚合时能直接覆盖正常客户端的贡献。这种攻击产生的恶意更新幅度远超正常更新，因此容易被基于更新范数的防御方法检测。

A3FL 攻击 [2] 通过在本地训练中引入对抗性优化目标，使生成的恶意更新在参数空间中与正常更新的分布更为接近。具体而言，A3FL 在训练恶意更新时，除了最小化后门任务损失外，还额外约束了恶意更新与预期正常更新之间的方向偏差，从而绕过仅依赖更新幅度或方向检测的防御。

DOBA 攻击 [3] 将后门目标分解到多个恶意客户端上协同优化。每个恶意客户端单独提交的更新幅度和方向都与正常更新接近，但这些更新在服务端取平均后，后门信号恰好得到增强，使得单看任何一个客户端的更新都不足以触发告警。

Neurotoxin 攻击 [4] 通过分析全局模型参数在正常训练中的更新频率，识别出那些被频繁修改的参数并刻意避开，转而将后门信息写入更新频率较低的参数区域。攻击停止后，后续正常训练主要更新高频参数，对低频参数中的后门信息覆盖有限，从而实现了持久性后门植入。

ReBA 攻击 [5] 在触发器设计和训练目标两个层面做了联合优化，自动搜索最优触发器位置和图案，同时调整训练超参数以在保持主任务精度的前提下最大化后门植入效率。

=== 联邦学习后门防御方法

已有防御方法从不同技术角度尝试在服务端识别和抑制恶意客户端更新。

FedAvg [6] 是标准联邦平均聚合算法，对每个采样客户端的更新赋予同等权重，不包含任何异常检测机制，在本作品中作为所有防御方法的对比基线。

Clip [7] 对每个客户端更新的 L2 范数设置上界，超出上限的更新被等比例缩放到上限值。该方法能有效拦截 ModelReplace 等大幅更新攻击，但无法区分方向正常而幅度合规的恶意更新与真正的良性更新。

DeepSight [8] 要求每个客户端提交模型输出层的权重和偏置分布，通过分析客户端模型在干净验证集上的输出分布和参数空间结构，利用聚类和异常检测技术识别后门模型。该方法对 Neurotoxin 等仅修改少量参数的持久性攻击检测效果有限。

FoolsGold [9] 记录每个客户端在多轮训练中的历史更新方向，计算客户端之间的余弦相似度矩阵。如果发现多个客户端的更新方向长期高度一致，则降低这些客户端的聚合权重。该方法的假设是攻击者为植入同一后门会提交方向一致的更新。但在非独立同分布场景中，良性客户端因数据不同导致更新方向自然存在差异，容易被误判为低相似度而被降权。

RFLBAT [10] 将模型结构分析和鲁棒聚合相结合，对客户端更新进行降维和聚类，通过排除离群簇来降低恶意更新的影响。该方法在部分攻击场景下效果良好，但对攻击策略持续演化的适应性有限。

PFedBA [11] 揭示了个性化联邦学习场景中的隐蔽后门攻击威胁，攻击者利用个性化层的隔离效应在客户端本地模型中持久驻留后门，而全局模型参数未见明显异常，进一步说明了现有防御方法在新型攻击范式下面临的挑战。
...
这个公式背后的假设是每个客户端都诚实训练，即使个别客户端的更新方向有所偏离，平均后也会被多数正常更新稀释。但后门攻击恰恰利用了这种信任，攻击者会刻意构造方向高度一致、精心控制幅度的恶意更新，使得平均之后后门信号非但没被稀释，反而被强化。更棘手的是，攻击者在训练恶意更新时同时约束了主任务损失，使得单纯依靠监测模型准确率来检测攻击的方法完全失效。本项目的实验也证实了这一点：即使后门攻击成功率高达 99%，主任务准确率仍然维持在 92% 以上。

本项目关注的五类攻击具有几个共同的隐蔽特征，这些特征使得单纯的幅度检测或准确率监控难以奏效。A3FL 和 DOBA 通过分布式协同和自适应优化，使每个恶意客户端单独提交的更新幅度保持在正常范围内，从而绕过基于范数阈值的防御，但当这些小幅更新在服务端被平均后，其指向相同后门目标方向的叠加效果仍足以在全局模型中植入后门。Neurotoxin 通过分析模型参数在正常训练中的更新频率分布，识别并避开高频更新的参数区域，将后门信息写入更新频率较低的参数，攻击停止后正常训练对低频参数中的后门信息覆盖不足，后门得以长期残留。A3FL 和 ReBA 等攻击在训练过程中根据全局模型状态动态调整攻击策略，包括触发器强度、优化目标和参数修改范围，使得基于固定规则或静态阈值的防御方法难以持续有效。

综合来看，每种基线防御方法都依赖单一维度的信号：Clip 只看更新幅度，FoolsGold 只看更新方向相似度，DeepSight 只看输出层分布。当攻击者的策略恰好规避了该维度的检测时，防御就失效了。鉴于后门攻击对联邦学习系统的严重危害性以及现有防御方法的上述局限性，有必要研究一种通过多维度特征融合实现鲁棒检测的防御方法。这指向一种基于自适应渐进鲁棒聚合的联邦学习后门对抗防御算法的设计核心：引入多维特征并通过渐进式流程整合，使各维度互为补充。

== 作品概述与创新点

=== 作品概述

一种基于自适应渐进鲁棒聚合的联邦学习后门对抗防御算法（Adaptive Progressive Robust Aggregation，以下简称 APRA）是本作品提出的联邦学习后门防御方法。它的核心思想是：不追求精确识别每一个恶意客户端，而是通过多阶段渐进式分析，逐步降低可疑客户端对全局模型更新的实际贡献。具体来说，APRA 依次对客户端更新进行多维特征提取、基于鲁棒统计的异常预筛、基于聚类分析的群体判断、以及基于信任权重的贡献控制，将传统的检测与剔除二元范式转化为粗筛、精选、控权的渐进式流程。详细的技术设计见第 2 章。

=== 创新点

本作品的创新点体现在四个层面。

第一，多阶段证据融合。将鲁棒统计过滤（MAD）、无监督聚类、相似度加权三种异质信号有机串联。MAD 处理更新幅度的明显异常，聚类处理客户端间的群体关系，信任加权处理个体贡献的精细控制。三个阶段各司其职，避免单一规则的失效导致整体防御崩溃。

第二，软降权替代硬过滤。不以二元的检测并排除恶意客户端为目标，而是以降低恶意更新对聚合结果的贡献为目标。这意味着即使某个恶意客户端未被完全剔除，其更新也会因信任权重较低而被大幅压缩，实际对全局模型的影响远低于其在聚合中的体量占比。

第三，自适应阈值调度。MAD 过滤的阈值 $k_t = k_"init" exp(-k_"decay" t)$ 随训练轮次指数衰减。训练早期模型参数变化剧烈，正常更新的方差大，需要宽松阈值避免误筛；训练后期模型趋于收敛，正常更新方向趋同，收紧阈值可更敏感地捕获异常。这种动态调度策略使防御强度与训练阶段自动匹配。

第四，全过程可审计追踪。每轮训练不仅输出聚合结果，还记录每个客户端的 MAD 分数、聚类标签、信任权重和裁剪因子，写入结构化 CSV 文件。这使得实验分析可以从最终指标回溯到每一轮、每个客户端的防御行为，回答防御为什么有效以及为什么误伤了某个良性客户端等问题。
...
APRA 的核心假设是：恶意客户端为了植入后门而构造的模型更新，最终都会在多个维度上留下可检测的痕迹。在更新幅度方面，恶意客户端如果试图直接替换全局模型，会产生远远超过正常水平的更新幅度，但 A3FL 和 DOBA 会刻意压制幅度至正常范围以规避检测。在输出层偏置方面，后门攻击需要将带触发器的样本导向目标类别，这会使得分类器输出层中目标类别对应的权重和偏置出现系统性偏移，不同于良性训练的自然分布。在噪声输入响应方面，给模型输入纯随机噪声，正常训练得到模型输出的类别概率接近均匀分布，但如果分类器被植入后门，目标类对应的神经元通道可能被异常强化，导致即使输入噪声，目标类概率也异常偏高。在客户端间方向关系方面，多个恶意客户端为植入同一后门，其更新在参数空间中往往指向相似的方向，而良性客户端因各自持有的 Non-IID 数据不同，更新方向自然分散。

单独的维度可能产生误判，例如 Non-IID 数据本身就会导致良性客户端更新方向存在差异，可能被误认为方向不一致。但多维信号合并后，恶意更新与多数正常更新之间的差异会被放大，形成可利用的判定边界。

=== 四阶段架构

基于上述思想，APRA 将防御拆解为四个串行阶段，每个阶段的输出是下一阶段的输入。

阶段一为多维特征提取。对每个被采样客户端，提取三类特征：选定分类器层的展平更新向量，作为方向特征；整模型更新的 L2 范数，作为幅度特征；可选地拼接 NBD 和 NDIF 行为探针，其中 NBD 计算客户端模型与全局模型在输出层偏置上的差异，NDIF 比较两者在随机噪声输入上的输出分布比值。最后通过 PCA 将拼接后的高维向量降维至 3 至 5 维，降低小样本下距离度量的噪声，实验中每轮仅采样 10 个客户端。

阶段二为自适应 MAD 预过滤。以所有被采样客户端更新幅度的中位数为中心、中位绝对偏差（Median Absolute Deviation, MAD）为尺度，计算每个客户端的修正 z-score。MAD 使用中位数而非均值作为中心估计，因为均值会被单个极大值拉偏而中位数保持鲁棒。阈值 $k_t = "max"(2.0, k_"init" "exp"(-k_"decay" t))$ 随轮次 $t$ 指数衰减：训练早期 $k_t$ 接近 $k_"init"$（默认 5.0），允许较大的更新幅度差异以容纳正常的模型探索；训练后期 $k_t$ 收窄至最小值 2.0，对异常更新更加敏感。若 MAD 过滤后剩余客户端数低于 $"max"(2, n/2)$，触发安全保留机制，保留最接近中位数的客户端，确保训练不会因过度过滤而停滞。

阶段三为层次聚类与可信簇选择。对通过 MAD 的客户端的降维特征向量，使用 Ward 链接的凝聚层次聚类。在 $k in [2, "max"_k]$ 范围内搜索最优簇数，以轮廓系数（Silhouette Score）评价每种划分的聚类质量，选择得分最高的 $k$。随后对每个簇计算得分，取簇内样本数与簇内平均余弦相似度的乘积，选取得分最高的簇作为可信簇。该策略确保选中规模大且内部方向一致的群体，而非简单取最大簇或最近邻。

阶段四为信任加权与自适应裁剪。计算每个可信簇内客户端之间的成对余弦相似度，取每个客户端与其他客户端的最大相似度作为其代表性得分，通过 logit 变换映射为信任权重 $w_i in [0,1]$。信任权重决定了两个关键量：每个客户端更新在最终聚合中的加权比例，以及其裁剪因子 $"clip"_i = "base_clip" times (w_i + epsilon) / ("max"(w) + epsilon)$。权重低的客户端不仅聚合占比小，其更新幅度也被更强地压缩，形成低信任、强裁剪、低贡献的双重抑制。

=== 设计取舍

#box[设计取舍][
APRA 的设计目标不是输出一个恶意客户端列表，而是控制每个客户端更新对全局模型的实际贡献。这种软降权策略比硬过滤有两个优势。其一是容错性：被误判的良性客户端最多被暂时降权，不会被完全剥夺训练参与权。其二是持续性抑制：未被前两阶段筛除的恶意客户端仍在加权和裁剪阶段被系统性限制贡献，而非一旦漏过就完全不受约束。
]
...
#box[设计思路][
APRA 将是否异常拆成两个视角：范数异常反映更新幅度是否过大或过小，适合检测 ModelReplace 等大幅度攻击；方向特征反映更新是否偏离正常类别边界，适合检测低幅度但方向可疑的攻击，如 A3FL、DOBA。二者配合能够覆盖更多攻击类型。
]
...
#box[设计思路][
NBD（Neuron Bias Difference）观察输出层偏置变化，NDIF（Neuron Distribution Imbalance Factor）使用随机噪声探测模型行为。随机噪声不是为了获得语义正确的分类结果，而是为了放大异常目标类偏好。正常模型对噪声输入通常不会稳定偏向同一类别；后门模型则可能因为目标类通道被强化而出现异常响应。
]
...
#box[设计思路][
这是一处关键的鲁棒性设计。防御算法不能只追求剔除可疑客户端，还必须保证训练过程持续推进。在 Non-IID 场景下，正常客户端的更新范数也可能差异较大；安全保留机制能降低单轮误判导致全局模型停滞或剧烈震荡的风险。
]
...
若某个簇人数多但内部方向混乱，其得分会因余弦相似度低而下降；若某个簇只有单个客户端，即使它距离特殊，也不会被视为可信主体。这一策略使 APRA 更偏向选择多数且方向一致的客户端群体，符合联邦学习中多数客户端为良性的基本假设。
...
这段代码的设计思路是从硬选择过渡到软降权。即使某个客户端通过了 MAD 和聚类，仍然会因为与可信群体方向不够一致而获得较低权重。logit 变换将相似度差异放大，使方向不一致的客户端获得明显更低的权重。
...
#box[设计思路][
APRA 的最终目标不是输出攻击者名单，而是控制每个客户端对全局模型的实际影响。MAD、聚类、信任权重和裁剪分别对应初筛、群体判断、贡献排序、幅度限制。四步串行后，即使某些恶意客户端没有被完全剔除，它们的更新也被限制在较小贡献内，从而在保证训练可继续的前提下最大化防御效果。
]
...
#figure(
  ...
  caption: [CIFAR-10 与 CIFAR-100 主任务准确率对比。APRA 在两个数据集上均保持与基线接近的准确率水平。],
)
...
- *阶段过滤行为与数据集无关*：MAD 剔除率、聚类增量剔除率和最终恶意入选率在 CIFAR-10 和 CIFAR-100 上完全一致。这说明 APRA 的前三个阶段（MAD + 聚类）的输出，即哪些客户端被保留、哪些被剔除，主要由客户端更新的统计特征和群体关系决定，与数据集的类别数无关。
...
- *良性误伤是 Non-IID 敏感度的指示器*：CIFAR-100 的良性误伤率约 54%，是 CIFAR-10（约 27%）的 2 倍。这并非方法缺陷，而是反映了 100 类 Non-IID 分配下良性客户端之间天然的更新方向差异。安全保留机制在此场景下发挥了关键作用，确保即使超过半数客户端被排除，训练仍可继续。降低聚类阶段对 Non-IID 良性客户端的误判，是进一步提升 APRA 性能的最直接方向。
...
- *层次聚类*处理客户端群体关系，识别多数且方向一致的可信群体，适合 Non-IID 数据下的分布式判断。
...
传统防御方法通常以检测并排除恶意客户端为目标。但在 Non-IID 联邦学习中，正常客户端之间的更新方向本身就有合理差异，硬过滤容易造成两类错误：将 Non-IID 良性客户端误判为恶意（误剔除），或将伪装良好的恶意客户端漏过（漏检）。

APRA 的策略转变是将目标从判定谁是恶意者变为控制每个客户端对聚合结果的实际贡献。这一设计带来两个优势：
...
本作品完成了 APRA（一种基于自适应渐进鲁棒聚合的联邦学习后门对抗防御算法）的完整设计与实现。APRA 通过在联邦学习服务端引入多维特征提取、自适应 MAD 预过滤、层次聚类与可信簇选择、信任加权与自适应裁剪四阶段渐进式流程，在保持模型正常分类能力的同时，有效抑制了后门攻击的成功率。
...
- 过程追踪数据验证了粗筛、精选、控权三阶段协同的有效性，MAD 处理幅度异常、聚类处理群体关系、加权裁剪控制个体贡献，三阶段缺一不可。

APRA 的工程价值在于将鲁棒统计、聚类分析和加权裁剪组合为可追踪、可解释的防御流程，并将防御目标从检测恶意客户端转变为渐进式降低可疑更新的聚合贡献，从而在准确率、防御效果和误伤率之间实现更合理的权衡。
...
*维护网络空间安全*：随着联邦学习在关键信息基础设施中的部署日益增多，后门攻击已成为网络安全防御的新战场。有效的后门防御方法能够保障分布式 AI 系统的模型完整性，防止攻击者通过控制边缘节点向核心模型注入恶意行为，从而维护国家级网络空间安全边界。

*保障金融数据隐私与模型安全*：金融行业是联邦学习最早也是最重要的应用领域之一。银行、保险等机构需要在跨机构联合建模（如联合风控、反洗钱）中保护客户隐私数据。本作品的方法可以在不访问原始数据的前提下，检测并抑制来自恶意参与方的模型投毒行为，保障联合建模结果的可靠性。

*支撑医疗健康领域的安全协同*：医疗数据具有极高的隐私敏感性和跨机构协同需求。多家医院联合训练疾病诊断模型时，必须防范恶意参与方植入可能危及患者安全的后门。APRA 的细粒度过程追踪能力在此场景中尤为重要，它可以追溯每一轮训练中每个参与方的行为，为医疗 AI 的安全合规提供审计依据。

*赋能自动驾驶与物联网安全*：自动驾驶和 IoT 设备通常作为联邦学习的边缘客户端参与训练，这些终端设备更容易被物理攻破或远程控制。APRA 通过多维特征的综合判断，不依赖单一假设来识别恶意更新，对终端安全防护薄弱的边缘场景具有更好的鲁棒性。
...
#text(size: 10.5pt)[
[1] Bagdasaryan E, Veit A, Hua Y, et al. How to backdoor federated learning[C]//Proceedings of the 23rd International Conference on Artificial Intelligence and Statistics (AISTATS). PMLR, 2020: 2938-2948.

[2] Zhang H, Jia J, Chen J, et al. A3FL: Adversarially adaptive backdoor attacks to federated learning[C]//Advances in Neural Information Processing Systems 36 (NeurIPS). Curran Associates, 2023.

[3] JIANG Yufei, TIAN Yulong. 基于触发差异优化的联邦学习持久后门攻击[J]. 计算机科学, 2025.

[4] Zhang Z, Panda A, Song L, et al. Neurotoxin: Durable backdoors in federated learning[C]//Proceedings of the 39th International Conference on Machine Learning (ICML). PMLR, 2022: 26429-26446.

[5] YANG Yilong, LIU Xinjing, WU Zefeng, et al. Catch me if you can: Retain high stealthiness and durability of backdoor attack in federated learning[J]. IEEE Transactions on Information Forensics and Security, 2026, 21.

[6] McMahan B, Moore E, Ramage D, et al. Communication-efficient learning of deep networks from decentralized data[C]//Proceedings of the 20th International Conference on Artificial Intelligence and Statistics (AISTATS). PMLR, 2017: 1273-1282.

[7] Sun Z, Kairouz P, Suresh A T, et al. Can you really backdoor federated learning?[C]//NeurIPS 2019 Workshop on Federated Learning for Data Privacy and Confidentiality. 2019.

[8] Rieger P, Nguyen T D, Miettinen M, et al. DeepSight: Mitigating backdoor attacks in federated learning through deep model inspection[C]//Proceedings of the 2022 Network and Distributed System Security Symposium (NDSS). Internet Society, 2022.

[9] Fung C, Yoon C J M, Beschastnikh I. The limitations of federated learning in Sybil settings[C]//Proceedings of the 23rd International Symposium on Research in Attacks, Intrusions and Defenses (RAID). Springer, 2020: 301-316.

[10] Wang Y, Zhai D, Zhan Y, et al. RFLBAT: A robust federated learning algorithm against backdoor attack[EB/OL]. arXiv:2201.03772, 2022.

[11] LYU Xiaoting, HAN Yufei, WANG Wei, et al. Lurking in the shadows: Unveiling stealthy backdoor attacks against personalized federated learning[C]//Proceedings of the 33rd USENIX Security Symposium (USENIX Security 24). USENIX Association, 2024.
]
