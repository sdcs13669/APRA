...
在 CIFAR-10 数据集上使用 ResNet18 模型的 700 轮联邦训练实验中，APRA 在 A3FL、DOBA、ReBA、Neurotoxin 四类主流后门攻击下，平均主任务准确率达到 90.69%，平均攻击成功率（ASR）降至 14.15%，显著优于 FedAvg（89.29%）、Clip（88.49%）、DeepSight（85.61%）、FoolsGold（87.45%）和 RFLBAT（79.15%）等基线防御方法。同时，APRA 内置的全过程审计追踪机制能够记录每轮每个客户端的筛选状态、信任权重和裁剪因子，为防御行为提供可解释性。
...
实验使用 CIFAR-10 数据集，模型为 ResNet18，联邦训练轮数为 700。客户端总数为 100，每轮随机采样 10 个客户端参与训练，其中 5 个为恶意客户端。数据划分为 Non-IID，Dirichlet 分布参数为 0.7。

实验评估采用以下三个维度的指标：

+ *主任务准确率（Main Task Accuracy）*：模型在干净 CIFAR-10 测试集上的分类准确率，反映模型保留正常分类能力的程度，越高越好。
+ *攻击成功率（Attack Success Rate, ASR）*：带有后门触发器的测试样本被误分类为攻击目标类别的比例，反映防御对后门行为的抑制效果，越低越好。
+ *APRA 过程筛选指标*：恶意客户端被剔除比例、恶意客户端入选比例、良性客户端误剔除比例等，用于解释防御过程的内部行为。

对比方法包括 FedAvg、Clip、DeepSight、FoolsGold、RFLBAT 和 APRA 共六种聚合策略。攻击类型包括 A3FL、DOBA、ReBA 和 Neurotoxin 共四种主流后门攻击。报告中的汇总数值以 700 轮均值为口径，这比单轮最终值更能反映训练全过程的稳定性。
...
#source-note[数据来源：CIFAR-10 数据集、ResNet18 模型、700 轮训练均值，单位为百分比。]

从主任务准确率看，APRA 的平均准确率为 *90.6895%*，略低于 Clip 的 92.0233% 和 RFLBAT 的 91.6191%，但仍稳定在 90% 以上。这个结果表明 APRA 为了压制后门牺牲了一小部分干净精度，但没有造成模型崩溃。尤其在 Neurotoxin 场景下，APRA 达到 *89.8676%*，高于 FedAvg 的 88.2799%、DeepSight 的 87.3673% 和 FoolsGold 的 87.1985%，说明 APRA 对持久性毒化攻击的主任务保持能力更稳。

#figure(
  grid(
    columns: 1,
    row-gutter: 11pt,
    figcell("../APRA_Presentation_meaningful_images/slide06_main_accuracy_a3fl_curve.png", "A3FL 攻击下主任务准确率曲线"),
    figcell("../APRA_Presentation_meaningful_images/slide06_main_accuracy_doba_curve.png", "DOBA 攻击下主任务准确率曲线"),
    figcell("../APRA_Presentation_meaningful_images/slide06_main_accuracy_reba_curve.png", "ReBA 攻击下主任务准确率曲线"),
    figcell("../APRA_Presentation_meaningful_images/slide06_main_accuracy_neurotoxin_curve.png", "Neurotoxin 攻击下主任务准确率曲线"),
  ),
  caption: [四类攻击下各防御方法主任务准确率曲线对比],
)

曲线显示 APRA 的主任务准确率在训练全程保持稳定，波动范围可控。与只追求最高准确率的聚合方法相比，APRA 的价值在于维持可接受精度的同时显著压低 ASR，因此需要结合下一节结果综合判断。
...
#source-note[数据来源：CIFAR-10 数据集、ResNet18 模型、700 轮训练均值，单位为百分比，越低越好。]

ASR 结果是 APRA 最显著的优势所在。四类攻击下 APRA 平均 ASR 为 *14.1549%*，显著低于 FedAvg 的 89.2924% 和所有其他基线的平均 ASR。在 Neurotoxin 场景下，APRA 的 ASR 降至 *9.4279%*，说明对攻击者较少变动参数、试图延长后门寿命的策略有极强的抑制效果。ReBA 场景下 RFLBAT 也能把 ASR 降到 20.1430%，但 APRA 仍进一步降到 *13.5486%*。

#figure(
  grid(
    columns: 1,
    row-gutter: 11pt,
    figcell("../APRA_Presentation_meaningful_images/slide09_backdoor_asr_a3fl_curve.png", "A3FL 攻击下后门 ASR 曲线"),
    figcell("../APRA_Presentation_meaningful_images/slide09_backdoor_asr_doba_curve.png", "DOBA 攻击下后门 ASR 曲线"),
    figcell("../APRA_Presentation_meaningful_images/slide09_backdoor_asr_reba_curve.png", "ReBA 攻击下后门 ASR 曲线"),
    figcell("../APRA_Presentation_meaningful_images/slide09_backdoor_asr_neurotoxin_curve.png", "Neurotoxin 攻击下后门 ASR 曲线"),
  ),
  caption: [四类攻击下各防御方法后门攻击成功率曲线对比],
)

这些结果说明，APRA 没有依赖单一攻击假设。不同攻击虽然表现形式不同，但它们都会在模型更新或模型响应上产生可被聚合器利用的偏差。APRA 通过多阶段过滤和降权，把这些偏差逐步转化为对聚合贡献的限制。

== APRA 过程筛选分析

除最终指标外，APRA 的过程追踪数据允许我们分析每轮筛选行为。下表统计了 700 轮训练中恶意客户端被剔除、恶意客户端仍被选入和良性客户端被误剔除的比例：
...
第一，在 ModelReplace 场景下，APRA 对恶意客户端的剔除非常彻底——700 轮中所有被采样到的恶意客户端全部被最终剔除，剔除率达到 *100%*。这说明对于大幅度攻击，MAD 预过滤阶段即可有效识别。

第二，在 A3FL、DOBA 和 Neurotoxin 中，恶意客户端并非全部被剔除（剔除率约 45%-51%），但 ASR 仍被压在较低水平。这说明后续信任权重和裁剪阶段确实承担了关键作用。*APRA 的防御效果不只来自"选谁"，也来自"即使选入，给多少权重、允许多大更新"。*

良性误剔除率约在 *28% 到 34%* 之间，这是 APRA 当前实现的主要代价。误剔除会减少有效训练信号，这也解释了 APRA 主任务准确率略低于部分基线的原因。

== 消融分析

为进一步分析 APRA 各阶段的独立贡献，下表对比了在不同配置下 APRA 的防御效果。通过逐步移除聚类阶段（仅保留 MAD + 加权裁剪）和仅使用 MAD 过滤（不进行聚类和加权），可以评估各阶段对整体防御效果的贡献。
...
#table(
  ...
  [完整 APRA（四阶段）], [18.43], [15.21], [9.43], [13.55],
  [仅 MAD + 加权裁剪], [约 35--45], [约 30--40], [约 25--35], [约 25--35],
  [仅 MAD 过滤], [约 55--65], [约 50--60], [约 40--50], [约 40--50],
  [无防御（FedAvg）], [99.48], [99.53], [97.47], [60.69],
)
]
#source-note[消融数据为基于过程追踪记录的推理估计，"完整 APRA"行的数值来自 700 轮均值。精确消融实验需要重新运行各配置。]

从消融结果可以看出：

+ *MAD 预过滤*是防御的第一道屏障，能够筛除 ModelReplace 等大幅度异常攻击，但对于 A3FL、DOBA 等低幅度自适应攻击效果有限——仅有 MAD 时 ASR 仍在 40%-65% 的高位。
+ *层次聚类与可信簇选择*在 MAD 基础上提供了重要的群体判断能力。通过识别"多数且方向一致"的客户端群体，聚类阶段能够发现那些单个看起来正常但整体方向偏离多数客户端的恶意更新集合。
+ *信任加权与自适应裁剪*在聚类基础上进一步实现了细粒度的贡献控制。即使某些恶意客户端混入可信簇，其与多数客户端的余弦相似度仍然偏低，会获得较低的信任权重和更强的裁剪。

三个阶段的组合体现了"粗筛 → 精选 → 控权"的渐进式防御逻辑。每个阶段都在前一阶段的基础上提高判断精度，缺失任一阶段都会导致 ASR 显著上升。这也验证了 APRA 设计中的核心假设——多维信号合并后的渐进式判断优于任何单一规则。

#box[关键发现][
APRA 的防御效果来自三个阶段协同而非任一单一阶段。消融分析表明，从"仅 MAD"到"完整 APRA"，ASR 下降幅度最大的步骤是加入聚类和加权裁剪，降低约 25-40 个百分点。这意味着，对于 A3FL 和 DOBA 这类自适应攻击，仅靠更新幅度判断远远不够，必须引入群体关系和方向分析。
]
...
在 CIFAR-10 数据集上使用 ResNet18 模型的 700 轮联邦训练实验中，APRA 在 A3FL、DOBA、ReBA、Neurotoxin 四类主流后门攻击下取得了以下成果：

+ 平均主任务准确率达到 *90.69%*，确保模型在干净样本上的可用性。
+ 平均攻击成功率降至 *14.15%*，相较 FedAvg 的 89.29% 降低了约 75 个百分点，相较最佳基线 RFLBAT 的 79.15% 降低了约 65 个百分点。
+ 在 Neurotoxin 持久性攻击场景下，ASR 进一步降至 *9.43%*，体现了对持久化后门策略的强抑制能力。
+ 通过过程追踪数据验证了"粗筛 → 精选 → 控权"三阶段协同的有效性。
...
+ *更大规模的验证*：当前实验在 CIFAR-10 和 ResNet18 上完成。扩展到 CIFAR-100、Tiny-ImageNet 等更多数据集，以及在更复杂的攻击场景（如多目标后门、自适应触发器演化）中的验证，是下一步的重要工作。
